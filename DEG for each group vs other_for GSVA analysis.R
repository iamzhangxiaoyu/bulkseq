...
createList <- function(group=NULL) {
  ...
  treatsamList[[1]] = intersect(tumorsam, names(group[group=="Veh"])) 
  treatnameList[1] <- "Veh" 
  ctrlnameList[1] <- "Others" 
  
  #A-2: 类2 vs 其他
  sampleList[[2]] = tumorsam
  treatsamList[[2]] = intersect(tumorsam, names(group[group=="RA"]))
  treatnameList[2] <- "RA"
  ctrlnameList[2] <- "Others"
  
  #A-3: 类3 vs 其他
  sampleList[[3]] = tumorsam
  treatsamList[[3]] = intersect(tumorsam, names(group[group=="NR"]))
  treatnameList[3] <- "NR"
  ctrlnameList[3] <- "Others"
 
   #A-4: 类4 vs 其他
  sampleList[[4]] = tumorsam
  treatsamList[[4]] = intersect(tumorsam, names(group[group=="NRRA"]))
  treatnameList[4] <- "NRRA"
...
DEfiles <- c("SKCM_deseq2_test_result.Veh_vs_Others.txt",
             "SKCM_deseq2_test_result.RA_vs_Others.txt",
             "SKCM_deseq2_test_result.NR_vs_Others.txt",
             "SKCM_deseq2_test_result.NRRA_vs_Others.txt")
...
msigdfFile = "c3.tft.v7.1.symbols.gmt"
n.top = 10
mode = "up" #"up"和"dn"二选一
...
annColors <- list(subtype=c("Veh"=jco[1],"RA"=jco[2],"NR"=jco[3],"NRRA"=jco[4]))
